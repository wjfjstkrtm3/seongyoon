...
/*
서블릿(servlet)
java파일로 웹 서비스를 할 목적으로 만든 파일 (SimpleController.java)
서블릿 조건 : request, response 객체 사용 가능

1. extends HttpServelt 반드시 상속(웹 환경에서 제공되는 요청, 응답) 처리 가능
...
6. 만들어진 순서
   servlet(UI(X)) -> jsp(Java코드 + 혼재(X)) -> MVC 패턴 -> Framework(Spring)
   
7. 최종 : Model2 기반의 MVC 패턴을 적용한 프로젝트

7.1 : JSP만 가지고 개발  > Model(DAO, DTO) + JSP> Model1
	  JSP 모든 [클라이언트의 요청]을 받고 -> 처리 담당 -> ...
	  
	  Model2 방식의 MVC
	  Model(DTO, DAO) >> 순수한 JAVA 클래스
	  View >> JSP(EL&JSTL) >> 화면구성
	    그럼 누가 클라이언트의 요청 받고 파악하고 처리하는 과정을 담당할까(servlet)
	  Controller > Servlet으로 작성  extends HttpServlet 상속받는 파일
	  1. 클라이언트의 요청을 파악(로그인, 게시판 글쓰기, 게시판 상세보기)
	  2. 요청 파악을 통해서 (다양한 MVC 적용) : DTO 객체 생성, DAO 객체 생성 ...
	  
	  
8. 서블릿 파일은 클라이언트에서 어떻게 요청할까	
	<form action = "login.ok.jsp" method = "GET">
	localhost:8090/WebServlet_1/login.ok.jsp?id=kglim&pwd=1004
...
8.2 서블릿 파일의 요청은
  ...
  <servlet-mapping>
	...
	최초 요청 -> servlet 컴파일 -> 실행 -> doGet() 호출, doPost() 호출 
	두번째 -> 실행 -> doGet() 호출, doPost() 호출
